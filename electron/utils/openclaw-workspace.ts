...
/**
 * Merge MClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeMClawContextOnce(): Promise<number> {
 ...
 * bootstrap files.
 */
export async function ensureMClawContext(): Promise<void> {
  let skipped = await mergeMClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeMClawContextOnce();
    if (skipped === 0) {
      logger.info(`MClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`MClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`MClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
