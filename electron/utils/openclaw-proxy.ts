...
interface SyncProxyOptions {
   ...
   * currently disabled in MClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync MClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because MClaw proxy is disabled and preserve mode is enabled');
      return;
    }
