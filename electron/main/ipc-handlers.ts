...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the MClaw chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via MClaw WebSocket chat events,
...
function registerCheckpointHandlers(): void {
  ...
  ipcMain.handle('checkpoint:create', async (_, params: {
    taskId: string;
    comment?: string;
    projectDir?: string;
  }) => {
  ...
  ipcMain.handle('checkpoint:list', async (_, taskId: string) => {
    try {
      ...
      return { success: false, error: String(error), checkpoints: [] };
    }
  });

  // Get a single checkpoint
  ipcMain.handle('checkpoint:get', async (_, checkpointId: string) => {
    try {
      const checkpoint = getCheckpoint(checkpointId);
      if (!checkpoint) {
        return { success: false, error: 'Checkpoint不存在' };
      }
  ...
  ipcMain.handle('checkpoint:rollback', async (_, params: {
    checkpointId: string;
    projectDir?: string;
  }) => {
    try {
      const result = rollbackCheckpoint(params.checkpointId, params.projectDir);
      return result;
    } catch (error) {
  ...
  ipcMain.handle('checkpoint:delete', async (_, checkpointId: string) => {
    try {
      const success = deleteCheckpoint(checkpointId);
      return { success };
    } catch (error) {
...
/**
 * Rules IPC handlers
 * Coding rules management - file-based storage
 */
function registerRulesHandlers(): void {
  const fsP = import('fs/promises');
  const path = import('path');

  // Get rules file path for a project
  const getRulesPath = async (projectDir?: string) => {
    const { join } = await path;
    const dir = projectDir || process.cwd();
    return join(dir, '.mclaw', 'rules.json');
  };

  // Initialize rules directory and file
  const initRules = async (rulesPath: string) => {
    ...
    if (!await fs.access(rulesPath).catch(() => false)) {
      const defaultRules = {
      ...
      await fs.writeFile(rulesPath, JSON.stringify(defaultRules, null, 2), 'utf8');
    }
  };

  // Get rules
  ipcMain.handle('rules:get', async (_, projectDir?: string) => {
    try {
      const fs = await fsP;
      const rulesPath = await getRulesPath(projectDir);
      await initRules(rulesPath);
  ...
  // Update rules
  ipcMain.handle('rules:update', async (_, { rules, projectDir }: { rules: unknown; projectDir?: string }) => {
    try {
      const fs = await fsP;
      const rulesPath = await getRulesPath(projectDir);
      await initRules(rulesPath);
  ...
  // Get rules as LLM prompt
  ipcMain.handle('rules:prompt', async (_, projectDir?: string) => {
    try {
      const fs = await fsP;
      const rulesPath = await getRulesPath(projectDir);
      await initRules(rulesPath);
      ...
      const prompt = `必须严格遵守以下编码规范：
1. 代码风格：${rules.codeStyle}
2. 命名规范：
   - 函数：${rules.naming?.function || 'camelCase'}
   - 类：${rules.naming?.class || 'PascalCase'}
   - 变量：${rules.naming?.variable || 'camelCase'}
   - 常量：${rules.naming?.constant || 'UPPER_CASE'}
3. 文件结构：${JSON.stringify(rules.fileStructure, null, 2)}
4. 技术栈：${rules.language} + ${rules.framework}`;

      return { success: true, prompt };
    } catch (error) {
      console.error('Failed to get rules prompt:', error);
      return { success: false, error: String(error), prompt: '' };
    }
  });
}
...
/**
 * Memory IPC handlers
 * Project memory management - file-based storage
 */
function registerMemoryHandlers(): void {
  // Get memory
  ipcMain.handle('memory:get', async (_, projectDir?: string) => {
    try {
  ...
  // Update memory
  ipcMain.handle('memory:update', async (_, { updates, projectDir }: { updates: unknown; projectDir?: string }) => {
    try {
      const result = await updateMemory(updates as Partial<import('../utils/memory').MemoryData>, projectDir);
      return result;
    } catch (error) {
  ...
  // Get memory as LLM prompt
  ipcMain.handle('memory:prompt', async (_, projectDir?: string) => {
    try {
      const prompt = await getMemoryPrompt(projectDir);
      return { success: true, prompt };
    } catch (error) {
...
function registerMcpHandlers(): void {
  // Start MCP server
  ipcMain.handle('mcp:start', async (_, config: import('../utils/mcp').McpServerConfig) => {
    try {
  ...
  // Call MCP tool
  ipcMain.handle('mcp:call', async (_, { serverId, toolName, args }: { serverId: string; toolName: string; args: Record<string, unknown> }) => {
    try {
  ...
  ipcMain.handle('mcp:list', async () => {
    try {
      const servers = getRunningMcpServers();
  ...
  ipcMain.handle('mcp:defaults', async () => {
    try {
      const servers = getDefaultMcpServers();
      return { success: true, servers };
    } catch (error) {
      console.error('Failed to get default MCP servers:', error);
      return { success: false, error: String(error), servers: [] };
    }
  });
}
