...
import { SendHorizontal, Square, X, Paperclip, FileText, Film, Music, FileArchive, File, Loader2, AtSign, ChevronDown, Sparkles, Zap, Brain, Target, Image, Video, Code, Presentation, Mic, MicOff, CheckCircle, XCircle } from 'lucide-react';
...
export function ChatInput({ onSend, onStop, disabled = false, sending = false, isEmpty: _isEmpty = false, quickReplies = [], onQuickReply, streamingTools = [] }: ChatInputProps) {
  ...
  const stagingAttachments = attachments.filter(a => a.status === 'staging').length;
  const hasStagingFiles = stagingAttachments > 0;
  ...
  return (
    <div
      className={cn(
        "p-2 w-full mx-auto transition-all duration-300 max-w-3xl",
      )}
    ...
    >
      <div className="w-full">
        {/* 消息队列指示条 - WorkBuddy 风格 */}
        {streamingTools.length > 1 && (
          <div className="mb-2 message-queue-bar">
            <div className="flex items-center gap-2">
              <span className="cb-status-dot" />
              <span className="text-[11px] text-muted-foreground">
                消息队列 ({streamingTools.filter(t => t.status === 'running').length} 执行中)
              </span>
              <div className="flex items-center gap-1 ml-auto">
                {streamingTools.slice(0, 8).map((tool) => (
                  <div
                    key={tool.id}
                    className={cn(
        ...
        {/* 工具调用记录 - 增强版 */}
        {streamingTools.length > 0 && streamingTools.length <= 1 && (
          <div className="mb-2 px-3 py-2 rounded-xl bg-gradient-to-r from-blue-50/60 via-indigo-50/30 to-blue-50/60 dark:from-blue-900/20 dark:via-indigo-900/10 dark:to-blue-900/20 border border-blue-200/50 dark:border-blue-500/30">
            <div className="flex items-center gap-2 mb-2">
              <div className="flex items-center gap-1.5">
                <span className="cb-status-dot" />
                <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">工具调用</span>
              </div>
            </div>
            <div className="space-y-1">
              {streamingTools.map((tool) => (
                <div
                  key={tool.id}
                  className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-[11px] bg-white/50 dark:bg-white/5"
                >
                  <div className={cn(
                    "shrink-0",
                    tool.status === 'completed' && "text-green-500",
                    tool.status === 'running' && "text-blue-500 animate-spin",
                  ...
                  <span className={cn(
                    "flex-1 truncate",
                    tool.status === 'completed' && "text-gray-600 dark:text-gray-400",
        ...
        {hasStagingFiles && totalAttachments > 1 && (
          <div className="mb-3 px-3 py-2 rounded-lg bg-muted/50">
            <TaskProgress step={readyAttachments} total={totalAttachments} />
            <p className="text-xs text-muted-foreground mt-1">
              正在上传文件... ({readyAttachments}/{totalAttachments})
            </p>
          </div>
        )}
