...
import { RefreshCw, Brain, Bot, Sun, Moon, Monitor, History, BookOpen, BrainCircuit, Blocks } from 'lucide-react';
import { CheckpointButton } from '@/components/checkpoint';
...
export const ChatToolbar = memo(function ChatToolbar() {
  ...
  // 点击历史消息，滚动到对应消息并关闭
  const handleSelectHistory = (msgId: string) => {
    useChatStore.getState().scrollToMessage(msgId);
  ...
  return (
    <div className="flex items-center gap-2">
            ...
            <ScrollArea className="h-72">
              <div className="p-2">
                {userMessages.map((msg, idx) => (
                  <button
                    key={msg.id || idx}
                    onClick={() => handleSelectHistory(msg.id)}
      ...
      {/* Checkpoint */}
      <CheckpointButton className="h-8 w-8" />
