...
import { AlertCircle, Bot, Check, Plus, RefreshCw, Settings2, Trash2, X, MessageSquare, Users } from 'lucide-react';
...
import type { AgentSession } from './AgentSessionCard';
...
export function Agents() {
  ...
  // Mock session data - in real implementation, this would come from a store or API
  ...
  return (
    <div data-testid="agents-page" className="flex flex-col -m-6 dark:bg-background h-[calc(100vh-2.5rem)] overflow-hidden">
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full p-10 pt-16">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'agents' | 'sessions')} className="flex-1 flex flex-col min-h-0">
          <TabsList className="w-fit mb-6 bg-muted/50">
            <TabsTrigger value="agents" className="gap-2 text-[13px]">
              <Users className="h-4 w-4" />
              Agents
            </TabsTrigger>
            <TabsTrigger value="sessions" className="gap-2 text-[13px]">
              <MessageSquare className="h-4 w-4" />
              Sessions
            </TabsTrigger>
          </TabsList>

          <TabsContent value="agents" className="flex-1 overflow-y-auto pr-2 pb-10 min-h-0 -mr-2 mt-0">
          ...
          <TabsContent value="sessions" className="flex-1 overflow-y-auto pr-2 pb-10 min-h-0 -mr-2 mt-0">
            <AgentSessionList
              sessions={sessions}
              selectedSessionId={selectedSessionId ?? undefined}
              onSelectSession={setSelectedSessionId}
              onDeleteSession={(sessionId) => {
                // In real implementation, this would call an API to delete the session
                toast.success(`会话 ${sessionId} 已删除`);
              }}
              onOpenChat={(sessionId) => {
                // In real implementation, this would navigate to the chat view
                toast.info(`打开会话 ${sessionId}`);
              }}
              onNewSession={() => {
                // In real implementation, this would create a new session
                toast.success('创建新会话');
              }}
            />
          </TabsContent>
