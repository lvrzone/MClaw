...
// ── Types ──────────────────────────────────────────────────────
type NavPage = 'chat' | 'models' | 'agents' | 'autodev' | 'channels' | 'skills' | 'cron' | 'settings' | 'logs' | 'config';
...
export function Sidebar() {
  ...
  const currentPage: NavPage = (() => {
    ...
    if (location.pathname.startsWith('/autodev')) return 'autodev';
    ...
    <>
      ...
      <div className="flex h-full">
        ...
        >
          ...
          {/* AutoDev */}
          <LeftNavButton
            icon={<Code2 className="h-[20px] w-[20px]" strokeWidth={1.8} />}
            label="AutoDev"
            active={currentPage === 'autodev'}
            onClick={() => handleIconNav('/autodev')}
          />
