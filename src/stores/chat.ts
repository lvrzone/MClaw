...
/**
 * 构建增强版提示词，注入 Rules 和 Memory
 */
async function buildEnhancedPrompt(userMessage: string): Promise<string> {
  const parts: string[] = [];

  // 1. 添加 Rules（编码规范）- 从文件系统获取
  try {
    const rulesPrompt = await useRulesStore.getState().getRulesPrompt();
    if (rulesPrompt) {
      parts.push('=== 编码规范 (Rules) ===');
  ...
  // 2. 添加 Memory（记忆）- 从文件系统获取
  try {
    const memoryPrompt = await useMemoryStore.getState().getMemoryPrompt();
    if (memoryPrompt) {
...
const IMAGE_CACHE_KEY = 'lclaw:image-cache';
...
function buildSessionSwitchPatch(
  state: Pick<
    ChatState,
    'currentSessionKey' | 'messages' | 'sessions' | 'sessionLabels' | 'sessionCustomLabels' | 'sessionLastActivity' | 'sessionUnreadCounts'
  >,
  ...
  return {
    ...
    sessionCustomLabels: leavingEmpty
      ? clearSessionEntryFromMap(state.sessionCustomLabels, state.currentSessionKey)
    ...
    sessionUnreadCounts: {
      ...
      [nextSessionKey]: 0, // Clear unread count for the new session
    },
    messages: [],
    streamingText: '',
...
export const useChatStore = create<ChatState>((set, get) => ({
    ...
    _loadSessionsInFlight = (async () => {
      try {
        ...
        if (data) {
          ...
          // NOTE: Only auto-generate labels if user hasn't set a custom label (sessionCustomLabels).
              ...
              sessionsToLabel.map(async (session) => {
                try {
                  ...
                  set((s) => {
                    ...
                    // Only auto-generate label if user hasn't set a custom label
                    const hasCustomLabel = s.sessionCustomLabels[session.key]?.trim();
                    if (firstUser && !hasCustomLabel) {
  ...
  // newSession() design that avoids sessions.reset to preserve history.
    ...
    if (currentSessionKey === key) {
      ...
      set((s) => ({
        ...
        sessionCustomLabels: Object.fromEntries(Object.entries(s.sessionCustomLabels).filter(([k]) => k !== key)),
        sessionLastActivity: Object.fromEntries(Object.entries(s.sessionLastActivity).filter(([k]) => k !== key)),
        sessionUnreadCounts: Object.fromEntries(Object.entries(s.sessionUnreadCounts).filter(([k]) => k !== key)),
        messages: [],
        streamingText: '',
        streamingMessage: null,
        streamingTools: [],
  ...
  newSession: () => {
    ...
    const { currentSessionKey, messages, sessions, sessionLastActivity, sessionLabels, sessionCustomLabels, sessionUnreadCounts } = get();
    // Only treat sessions with no history records and no activity timestamp as empty
    const leavingEmpty = !currentSessionKey.endsWith(':main')
      ...
      && !sessionUnreadCounts[currentSessionKey];
    // When creating a session for a specific agent (handleNewSession sets currentAgentId first),
    // use that agent's prefix so the new session belongs to the correct agent, not the old one.
    const { currentAgentId } = get();
    const agentIdNorm = normalizeAgentId(currentAgentId);
    // If it's a non-main agent, use agent: prefix. For main, use canonical prefix (not old session's prefix).
    const prefix = agentIdNorm && agentIdNorm !== 'main'
      ? `agent:${agentIdNorm}`
    ...
    set((s) => ({
      ...
      sessionLabels: leavingEmpty
        ...
        : s.sessionLabels,
      sessionCustomLabels: leavingEmpty
        ? Object.fromEntries(Object.entries(s.sessionCustomLabels).filter(([k]) => k !== currentSessionKey))
        : s.sessionCustomLabels,
      sessionLastActivity: leavingEmpty
        ? Object.fromEntries(Object.entries(s.sessionLastActivity).filter(([k]) => k !== currentSessionKey))
        : s.sessionLastActivity,
      sessionUnreadCounts: leavingEmpty
        ? Object.fromEntries(Object.entries(s.sessionUnreadCounts).filter(([k]) => k !== currentSessionKey))
        : s.sessionUnreadCounts,
      messages: [],
      streamingText: '',
      streamingMessage: null,
      streamingTools: [],
  ...
  cleanupEmptySession: () => {
    ...
    set((s) => ({
      ...
      sessionCustomLabels: Object.fromEntries(
        Object.entries(s.sessionCustomLabels).filter(([k]) => k !== currentSessionKey),
      ),
      sessionLastActivity: Object.fromEntries(
        Object.entries(s.sessionLastActivity).filter(([k]) => k !== currentSessionKey),
      ),
      sessionUnreadCounts: Object.fromEntries(
        Object.entries(s.sessionUnreadCounts).filter(([k]) => k !== currentSessionKey),
      ),
    }));
  },
  ...
  loadHistory: async (quiet = false) => {
    ...
    const loadPromise = (async () => {
      ...
      // displayName (e.g. the configured agent name "MClaw") instead.
      // Also skip if user has set a custom label (sessionCustomLabels).
      const isMainSession = currentSessionKey.endsWith(':main');
      const hasCustomLabel = get().sessionCustomLabels[currentSessionKey]?.trim();
      if (!isMainSession && !hasCustomLabel) {
  ...
  sendMessage: async (
    ...
    // 注入 Rules 和 Memory 到消息中
    const enhancedText = await buildEnhancedPrompt(trimmed);
    ...
    // Only if user hasn't set a custom label
    const { sessionLabels, sessionCustomLabels, messages } = get();
    const isFirstMessage = !messages.slice(0, -1).some((m) => m.role === 'user');
    const hasCustomLabel = sessionCustomLabels[currentSessionKey]?.trim();
    if (!currentSessionKey.endsWith(':main') && isFirstMessage && !sessionLabels[currentSessionKey] && !hasCustomLabel && trimmed) {
    ...
    let _checkStuckTimer: ReturnType<typeof setTimeout> | null = null;
    const checkStuck = () => {
      const state = get();
      if (!state.sending) return;
      if (state.streamingMessage || state.streamingText) return;
      if (state.pendingFinal) {
        _checkStuckTimer = setTimeout(checkStuck, 10_000);
        return;
      }
    ...
    _checkStuckTimer = setTimeout(checkStuck, 30_000);
    ...
    try {
        ...
        result = await hostApiFetch<{ success: boolean; result?: { runId?: string }; error?: string }>(
          '/api/chat/send-with-media',
          {
            ...
            body: JSON.stringify({
              ...
              message: enhancedText || 'Process the attached file(s).',
              deliver: false,
              idempotencyKey,
      ...
      if (isRecoverableChatSendTimeout(errStr)) {
        ...
        clearHistoryPoll();
        if (_checkStuckTimer) { clearTimeout(_checkStuckTimer); _checkStuckTimer = null; }
  ...
  handleChatEvent: (event: Record<string, unknown>) => {
    ...
    if (eventSessionKey != null && eventSessionKey !== currentSessionKey) {
      // For OTHER sessions: increment unread count if there's a new assistant message
      const eventMsg = event.message as RawMessage | undefined;
      if (eventSessionKey && eventMsg?.role === 'assistant' && eventState === 'final') {
        set((s) => ({
          sessionUnreadCounts: {
            ...s.sessionUnreadCounts,
            [eventSessionKey]: (s.sessionUnreadCounts[eventSessionKey] || 0) + 1,
          },
        }));
      }
      return;
    ...
    switch (resolvedState) {
      ...
      case 'final': {
        ...
        if (finalMsg) {
          // Determine if there's actual output content (text, images, etc. - not just tool calls)
          const hasOutput = hasNonToolAssistantContent(finalMsg);
          const updates = collectToolUpdates(finalMsg, resolvedState);
          if (isToolResultRole(finalMsg.role)) {
            // ─── Tool result handling (unchanged logic) ───
            ...
            // If this tool result has actual output content, stop sending state
            if (hasOutput) {
              set({ sending: false, activeRunId: null, pendingFinal: false });
            }
            // Determine if this is a tool-only turn (no real text/image output)
            const toolOnly = !hasOutput;
            // Capture stream ID from the preserved reference (set BEFORE the first set() call
            // where streamingMessage gets cleared, so this still holds the live object)
            const streamSnapId = (currentStreamForPath as RawMessage | null)?.id || null;
            ...
            set((s) => {
              ...
              // Remove any snapshot messages that were added during tool processing,
              // as they will be replaced by the final complete response
              const currentStream = s.streamingMessage as RawMessage | null;
              // Use the same streamSnapId to ensure consistent filtering
              const streamId = currentStream?.id || streamSnapId;
              const filteredMessages = streamId
                ? s.messages.filter(m => m.id !== streamId)
                : s.messages;

              return toolOnly ? {
                messages: [...filteredMessages, msgWithImages],
                streamingText: '',
                streamingMessage: null,
                pendingFinal: true,
                streamingTools,
          ...
          } // closes if (isToolResultRole)
          } else {
            // Normal assistant message (not tool result) - add to history and reset sending
            const hasOutput = hasNonToolAssistantContent(finalMsg);
            if (hasOutput) {
              set({
                streamingText: '',
              ...
              // No output content - just clear streaming state
  ...
  // ── Create group chat (placeholder - not implemented) ──

  createGroupChat: (_agentIds: string[]) => {
    console.warn('createGroupChat is not yet implemented');
  },
