...
import type { AttachedFileMeta, RawMessage, ContentBlock } from './types';
import type { ChatGet, ChatSet } from './store-api';

/** Extract thinking content from a message (inline helper to avoid circular deps) */
function extractThinkingContent(msg: RawMessage): string | null {
      ...
      switch (resolvedState) {
        ...
        case 'final': {
          ...
          if (finalMsg) {
            ...
            // Also save the thinking content for display in the thinking panel.
            if (hasOutput && !toolOnly) {
              clearHistoryPoll();
              // 保存思考内容到 lastThinking 字段
              const thinkingContent = finalMsg ? extractThinkingContent(finalMsg) : null;
              set({ lastThinking: thinkingContent });
