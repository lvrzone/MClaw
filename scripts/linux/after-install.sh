...
# Post-installation script for MClaw on Linux
...
# Create symbolic link for MClaw app binary
if [ -x /opt/MClaw/clawx ]; then
    ln -sf /opt/MClaw/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/MClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/MClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/MClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/MClaw/resources/apparmor-profile'
...
echo "MClaw has been installed successfully."
